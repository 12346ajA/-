<template>
  ...
  <el-dialog title="添加分类" :visible.sync="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form size="small" :model="form" :rules="formRules" ref="form">
      ...
      <el-input v-model="form.name" placeholder="请输入分类名称" v-if="!showDateSelector"></el-input>
      <el-date-picker v-model="form.date" type="date" placeholder="选择日期" @change="updateDate" v-if="showDateSelector"></el-date-picker>
    </el-form-item>
  ...
  export default {
    ...
    methods: {
         updateDate() {
            ...
            this.form.name = `${year}-${month}-${day}`; // 将选择的日期赋给分类名称，格式为年月日
          },
      /*添加用户*/
      addUser() {
        let self = this;
